...
def infer_slot_from_text(text: str) -> str | None:
    ...
    if _YESNO_PAT.search(t) or t.endswith("吗？") or t.endswith("吗?"):
        return "yesno"
...
def parse_yesno(user_text: str) -> bool | None:
    ...
    neg_phrases = [
        "没有", "没", "不行", "不能", "不会", "不可以",
        "还是", "还在", "没变", "没有变", "没变少",
        "更多", "变多", "越来越多", "止不住", "还在流血", "继续流血",
        "还是头晕", "还是很晕", "还是疼", "还是很疼", "还是喘", "还在晃"
    ]
    ...
    # 超短口语
    yes = {"可", "可以", "行", "好", "好的", "有的", "嗯", "对", "是", "能", "会", "行的", "有"}
    no = {"不", "无"}
    if t in yes:
        return True
    if t in no:
